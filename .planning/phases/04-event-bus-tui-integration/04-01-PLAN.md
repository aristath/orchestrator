...
must_haves:
  truths:
    ...
    - "SubscribeAll returns a single channel that receives events from ALL topics"
  artifacts:
    - path: "internal/events/bus.go"
      provides: "Channel-based pubsub event bus with topic subscriptions and SubscribeAll"
      exports: ["EventBus", "NewEventBus", "Subscribe", "SubscribeAll", "Publish", "Close"]
...
- `SubscribeAll(bufSize int) <-chan Event` -- creates a SINGLE buffered channel and registers it as a subscriber for EVERY existing AND future topic. Implementation: add a dedicated `allSubs []chan Event` field to EventBus. In `Publish()`, after sending to topic-specific subscribers, also iterate `allSubs` and non-blocking send to each. In `Close()`, close allSubs channels too. This lets a consumer receive events from all topics on one channel without needing to merge multiple channels. If bufSize <= 0, default to 256.
- `Publish(topic string, event Event)` -- non-blocking send with `select/default` (drop if channel full). Return immediately if closed. Use RLock for concurrent publishers. After sending to topic-specific subs, also send to all `allSubs` channels.
- `Close()` -- set closed=true, close all subscriber channels (both topic-specific and allSubs). Use write lock. Idempotent (safe to call twice).
...
7. `TestSubscribeAll` -- call SubscribeAll, publish events on TopicTask and TopicDAG, verify the single channel receives events from BOTH topics
...
`go test -race -v ./internal/events/...` -- all 7 tests pass with no race conditions.
  </verify>
  <done>
EventBus with topic-based subscriptions, SubscribeAll for cross-topic consumption, non-blocking publish, and clean shutdown exists in internal/events/. All 7 tests pass under -race flag.
  </done>
</task>
...
5. SubscribeAll receives events from all topics on a single channel
6. ParallelRunner publishes events at all task lifecycle points
7. Setting EventBus=nil does not change existing behavior
</verification>

<success_criteria>
- internal/events/ package exists with EventBus, SubscribeAll, 6 event types, and passing tests
