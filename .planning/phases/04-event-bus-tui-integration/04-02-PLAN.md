...
must_haves:
  ...
  key_links:
    - from: "internal/tui/model.go"
      ...
      via: "Event subscription in Init()"
      pattern: "Subscribe"
    - from: "internal/tui/model.go"
...
Constructor: `New(eventBus *events.EventBus) Model` -- subscribe to TopicTask and TopicDAG (buffer 256). Store the subscription channel.
...
IMPORTANT: The event bus from Plan 01 uses topic-based subscriptions. The TUI needs to subscribe to BOTH TopicTask and TopicDAG. Either subscribe to both topics separately and merge them with a goroutine, or add a `SubscribeAll(bufSize int) <-chan Event` method to EventBus if needed. The simpler approach: subscribe to a single wildcard topic "*" OR just subscribe to both and use a select in the waitForEvent function. Choose the simplest approach that works.
...
`go test -race ./internal/...` -- no regressions in existing tests.
  ...
  <done>
TUI package exists with root model, agent pane (list + viewport), DAG pane, styles, and keybindings. Layout renders split-pane view. Event subscription wired. Focus navigation works with Tab/Shift+Tab/1/2/3. All code compiles and existing tests pass.
  </done>
</task>
